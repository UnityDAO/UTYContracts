...
contract ObelixToken {
    ...
    uint public _stakedTotalSupply;
    ...
    address public UTYToken;
    
    ObelixFarming public obelixFarming;
    ...
    constructor() public {
        ...
        _balances[owner] = _totalSupply; // Add all tokens to issuer balance
    }
    
    function DistributeProfitsOBELIX() external {
        require(EnableProfitDistribution, "Distribution is disabled");
        require(!getProfitsDistributed(msg.sender), "Profits already distributed");
        
        Staker memory staker = stakers[msg.sender];
        
        uint daysStaked = block.timestamp.sub(staker.startTimestamp) / 86400;
        
        require(daysStaked >= 14, "You must stake for 14 days to claim profits");
        
        uint distribution = AmountToDistribute / 2;
        
        uint eth = mulDiv(distribution, staker.stakedBalance, _stakedTotalSupply.add(_balances[UniswapPair]));
    ...
    function StakeOBELIX(uint amount) external {
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        Staker storage staker = stakers[msg.sender];
        if (staker.startTimestamp == 0 || staker.stakedBalance == 0) {
            staker.startTimestamp = block.timestamp;
        } else {
            uint percent = mulDiv(1000000, amount, staker.stakedBalance);      // This is not really 'percent' it is just a number that represents the totalAmount as a fraction of the recipientBalance
            if(percent.add(staker.startTimestamp) > block.timestamp) {         // We represent the 'percent' or 'penalty' as seconds and add to the recipient's unix time
    ...
    function UnstakeOBELIX(uint amount) external {
        Staker storage staker = stakers[msg.sender];
    ...
    function DistributeProfitsOBELIXFund() external {
        ...
        require(EnableProfitDistribution, "Distribution is disabled");
        uint eth = mulDiv(AmountToDistribute, 8, 100);
        OBELIXFund.transfer(eth);
    }
    
    function DistributeMaximusFounder() external {
        require(msg.sender == owner || msg.sender == Maximus || msg.sender == Founder);
        require(EnableProfitDistribution, "Distribution is disabled");
        uint eth = mulDiv(AmountToDistribute, 5, 100);
        Maximus.transfer(eth / 2);
        Founder.transfer(eth / 2);
    }
    
    function DistributeBuybacks() external {
        require(msg.sender == owner || msg.sender == buybacksUTY || msg.sender == buybacksOBELIX);
        require(EnableProfitDistribution, "Distribution is disabled");
        uint eth = mulDiv(AmountToDistribute, 7, 100);
    ...
    function DistributeProfitsOBELIXFarmer() external {
        require(EnableProfitDistribution, "Distribution is disabled");
        require(!getProfitsDistributedFarmers(msg.sender), "Profits already distributed to farmer");
        
        (uint tokens, uint startTimestamp) = obelixFarming.estimateOBELIXProvidedWithStartTimestamp(msg.sender);
        
        uint daysStaked = block.timestamp.sub(startTimestamp) / 86400;
        
        require(daysStaked >= 14, "You must stake for 14 days to claim profits");
        
        uint distribution = AmountToDistribute / 2;
        
        uint eth = mulDiv(distribution, tokens, _stakedTotalSupply.add(_balances[UniswapPair]));
    ...
    function DistributeProfitSenateCouncil() external {
        ...
        require(EnableProfitDistribution, "Distribution is disabled");
        uint eth = mulDiv(AmountToDistribute, 30, 100);
    ...
    function balanceOf(address owner) public view returns (uint256) {
        return _balances[owner].add(stakers[owner].stakedBalance);
    }
    
    function balanceOfNotStaked(address owner) public view returns (uint256) {
    ...
    function getProfitsDistributed(address holder) internal view returns(bool) {
        bytes32 key = keccak256(abi.encodePacked(currentProfitsDistributed, holder));
        return profitsDistributed[key];
    }
    
    function getProfitsDistributedFarmers(address holder) internal view returns(bool) {
        bytes32 key = keccak256(abi.encodePacked(currentProfitsDistributed, holder));
