...
contract LiquidityPool {
    ...
    constructor() public {
        ...
        obeliskToken.approve(address(uniswapV2), 100000000000E18);
    }
    ...
    function addLiquidity(uint _numTokensToInvest) external payable {      // This allows you to add liquidity without the burn penalty
        obeliskToken.transferFrom(msg.sender, address(this), _numTokensToInvest);
        (uint amountToken, uint amountETH, uint liquidity) = uniswapV2.addLiquidityETH{value: msg.value}(address(obeliskToken), _numTokensToInvest, 0, 0, address(this), now+86400);  // Adding liquidity to Uniswap via the router
    ...
    function reinvestRewards(bool locked, uint _numTokensToReinvest) external payable {
        ...
        if(locked) {
            ...
            (uint amountToken, uint amountETH, uint liquidity) = uniswapV2.addLiquidityETH{value: msg.value}(address(obeliskToken), _numTokensToReinvest, 0, 0, address(this), now+86400);  // Adding liquidity to Uniswap via the router
            thisStaker.poolTokenBalance = thisStaker.poolTokenBalance.add(liquidity);
            
        } else {
            uint numTokensToReinvest = calculateTokensOwed(msg.sender);
            thisStaker.lastTimestamp = block.timestamp;
            (uint amountToken, uint amountETH, uint liquidity) = uniswapV2.addLiquidityETH{value:msg.value}(address(obeliskToken), numTokensToReinvest, 0, 0, address(this), now+86400);
            thisStaker.poolTokenBalance = thisStaker.poolTokenBalance.add(liquidity);
        }
    }
    ...
    function calculateMonthlyYield() public view returns (uint256) {
        uint tokensInPool = obeliskToken.balanceOf(address(uniswapPair));
    ...
    function estimateOBELIXProvidedWithStartTimestamp(address _staker) external view returns (uint256, uint256) {
        uint tokensInPool = obeliskToken.balanceOf(address(uniswapPair));
        uint tokens = mulDiv(tokensInPool, stakers[_staker].poolTokenBalance, uniswapPair.totalSupply());
    ...
    function updateUinswapV2(address _uniswapv2) external onlyOwner {
        ...
        obeliskToken.approve(address(uniswapV2), 100000000E18);
    }
